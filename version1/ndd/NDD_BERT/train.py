...
class MainModel(BertPreTrainedModel):
    def __init__(self, config, loss_fn=None):
        ...
        self.num_labels = num_labels  # For binary classification
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, labels, device):
        ...
        output = output[:, 0, :]  # Use the first token ([CLS]) for classification
        classifier_out = self.classifier(output)
        main_prob = F.softmax(classifier_out, dim=1)

        # Ensure labels are reshaped to match the expected shape for torch.gather
        labels = labels.view(-1, 1)  # Reshape to (batch_size, 1)
        main_gold_prob = torch.gather(main_prob, 1, labels)  # Gather the probabilities corresponding to the true class labels

        loss_main = self.loss_fn.forward(main_gold_prob)  # Compute loss based on gathered probabilities
...
# Ensure padding and truncation are done properly
def collate_fn(batch):
    ...
    # Truncate or pad sequences to 512 tokens if needed
    input_ids = input_ids[:, :512]  # Ensure input length is at most 512
    attention_mask = attention_mask[:, :512]  # Ensure attention mask length is at most 512
    token_type_ids = token_type_ids[:, :512]  # Ensure token type ids length is at most 512
...
def main():
    ...
    output_model_path = args.output_model_directory
    output_tokenizer_path = args.output_tokenizer_directory
    ...
    val_file_path = os.path.join(input_path, args.dataset_name, 'val.csv')
    test_file_path= os.path.join(input_path, args.dataset_name, 'dev.csv')

    
    
    model = MainModel.from_pretrained("bert-base-uncased", num_labels = num_labels, loss_fn = LossFunction())
    device = 'cuda' if cuda.is_available() else 'cpu'
    model.to(device)
    optimizer = torch.optim.Adam(params=model.parameters(), lr=LEARNING_RATE)
    
    train_data = load_qqp(file_path=train_file_path, tokenizer=tokenizer)
    print("train data length", train_data.len)
    val_data = load_qqp(file_path=val_file_path, tokenizer=tokenizer)
    print("val data length", val_data.len)
    test_data = load_qqp(file_path=test_file_path, tokenizer=tokenizer)
    print("test data length", test_data.len)
    ...
    eval_dataloader = DataLoader(val_data, shuffle=False, batch_size=BATCH_SIZE, collate_fn=collate_fn)
    ...
    for epoch in range(num_epochs):
        ...
        end_time = time.time()  
        print(f'time for epoch {epoch+1}:{end_time-start_time}')
        print(f'\tValidation loss: {validation_loss}')
        print(f'\tValidation accuracy for epoch {epoch+1}: {eval_acc}')
        
        test_acc, pred_lst , prob_lst = inference(model, test_dataloader, tokenizer, device)
