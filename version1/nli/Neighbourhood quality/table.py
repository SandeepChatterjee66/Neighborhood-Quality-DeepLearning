...
def extract_epochs_required(val_accuracies, neighborhood_quality, patience=2):
    """Determine the epoch where early stopping occurs for each method with patience."""
    def find_early_stop(acc_list, patience):
        best_epoch = 0
    ...
    traditional_epochs = find_early_stop(val_accuracies, patience)
    nq_epochs = find_early_stop(neighborhood_quality, patience)
