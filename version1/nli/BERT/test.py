...
input_path = "./"
BATCH_SIZE = 64
snli_path = "./dataset/snli_1.0/snli_1.0_test.txt"
...
if not os.path.exists(snli_path):
    # Give a warning if the file or directory does not exist
    warnings.warn(f"Warning: The path '{snli_path}' does not exist!")
    ...
    for idx, batch in enumerate(tqdm(dataloader, ncols=100)):
        ...
        if data == "hans":
            predicted_labels = torch.where(
                ...
                torch.ones_like(predicted_labels),
                predicted_labels,
            )
        ...
        tmp_test_accuracy = accuracy_score(
...
def main():
    ...
    parser.add_argument("--snli_test_path", type=str, required=True)
    ...
    model.to(device)
    snli_data = load_snli(file_path=snli_path, tokenizer=tokenizer)
    snli_test_dataloader = DataLoader(snli_data, shuffle=False, batch_size=BATCH_SIZE)
    snli_test_accuracy, snli_pred_lst, snli_prob_lst = inference(
        model, snli_test_dataloader, tokenizer, device
    )
    print(f"\tSNLI set test accuracy: {snli_test_accuracy}")
    os.makedirs(os.path.dirname(snli_pred_lst), exist_ok=True)
    generate_prediction_file(snli_pred_lst, "./Predictions/SNLI/pred_snli.txt")

    os.makedirs(os.path.dirname(snli_prob_lst), exist_ok=True)
